...
compute shaders on top of `wgpu`. This is the full record of getting it fast,
including the two diagnoses that were wrong before the right one.

The short version: the batched backward pass went from losing most of the
CPU/GPU crossover to winning **every cell of it**, and the thing that was
actually slow was not what the profile first suggested, twice.

```sh
cargo run --release --bin gpu_bench     # reproduce every table below
./scripts/collect-gpu-report.sh          # + hardware, tests, regression check
```

---

Correctness is defined as agreement with the CPU, and checked: forward
matches to **1e-6**, gradients to **2e-6** relative.
